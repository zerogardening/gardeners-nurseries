...
(function (ZG) {
  ...
  /* ── 업무 창 ── 새로 등록하거나 고친다 (2026-09-17 우람님이 통째로 다시 잡으셨다).
     ...
     🔴 날짜 고르개는 **채팅에서 올릴 때만** 둔다 (2026-09-18 우람님).
        달력 탭은 위 달력에서 날을 고르고 ＋ 를 누르는 흐름이라 칸이 필요 없다.
        채팅에는 그 달력이 없어서 늘 오늘로만 잡혔다 — 그래서 여기만 칸을 연다.
  ...
  function 업무창(있던것, 채팅줄것) {
    ...
    /* ── 날짜 ── 채팅에서 올릴 때만 나온다. 머리글도 따라 바뀐다 */
    var 머리 = 만들기('h4', { text: (있던것 ? '업무 고치기' : '새 업무') + ' · ' + 날글(날) });
    var 날칸 = null;
    if (채팅줄것) {
      날칸 = 만들기('input', { class: 'inp num', type: 'date', value: 날 });
      날칸.addEventListener('change', function () {
        if (!날칸.value) { 날칸.value = 날; return; }   // 비워 두면 못 잡는다 — 되돌린다
        날 = 날칸.value;
        머리.textContent = '새 업무 · ' + 날글(날);
      });
    }
    ...
    var 상자 = 만들기('div', {
      ...
      머리,
      날칸 ? 만들기('div', { class: 'field' }, [만들기('label', { text: '날짜' }), 날칸]) : null,
