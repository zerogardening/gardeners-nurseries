...
(function (ZG) {
  ...
  /* 옛 자료에 남아 있는 특성을 지우지 않으려고 비교만 한다 — 화면에는 더 이상 안 낸다 */
  function 특성글(값) {
  ...
  function 저장() {
    ...
    var 바뀐것 = {};
    ['유통명', '학명', '규격', '매입단가', '과세구분', '상태', '판매단위'].forEach(function (k) {
  ...
  function 선택삭제() {
    ...
    본문 += 붙은것.length
      ? '<br><br>이 품목에 ' + 붙은것.join(' · ') + '이 남아 있습니다.<br>' +
        '<b>기록은 지우지 않습니다</b> — 매입 · 명세서는 세금계산의 근거라 그대로 둡니다.<br>' +
        '재고 목록에서만 안 보이게 됩니다.'
