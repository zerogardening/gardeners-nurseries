...
(function (ZG) {
  ...
  /* 🔴 `이름` 은 해시이자 열쇠다 — `#입고`·`#재고` 로 걸린 링크가 여기저기 있다.
     상품 화면은 들어오면 입고가 먼저다 (2026-09-17 우람님). */
  var 탭들 = [
    { 이름: '입고', 폰: true, 모듈: function () { return ZG.입고; } },
    { 이름: '재고', 폰: true, 모듈: function () { return ZG.재고; } }
  ];
  ...
  function 폰뼈대() {
    ...
    /* 🔴 아래 탭바의 이름은 「상품」이다(2026-09-17 우람님). 이 화면 안 상단탭 이름 '입고'·해시 '#입고' 와는 다른 것이다 */
    var 바 = u.탭바('상품', function (이름) {
      if (이름 !== '상품') return false;
      탭으로('입고');          // 「상품」을 다시 눌러도 페이지를 새로 열지 않는다
      return true;
    });
  ...
  function 시작() {
    ...
    window.addEventListener('storage', function (e) {
      if (e.key && e.key.indexOf('gn.v1.') === 0) 다시그리기();
    });
  }
