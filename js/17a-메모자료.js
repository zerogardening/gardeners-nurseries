/* 17a-메모자료 — 메모 · 영농일지 자료 + 사진 (12단계 설계 §5·§6 · 세부설계 §3-1·§3-3)
   표는 하나(gn.v1.메모)다. 한 장이 한 줄이라 두 사람이 다른 메모를 같은 시각에 저장해도 서로를 안 덮는다.
...
(function (ZG) {
  ...
  /* 🔴 일지 id 는 「그 날 + 그 사람」이다 (2026-09-17 우람님: 일지도 개인화).
     전에는 날짜만 못박아 `일지-2026-09-17` 이었다 — 그러면 두 사람이 같은 날에 쓸 때
     뒤에 쓴 사람이 앞사람 일지를 **통째로 덮는다**. 사람을 id 에 넣어 각자 한 장씩 갖는다.
     메일은 ASCII 로 눌러 쓴다 — 사진 경로(경로키)가 id 를 그대로 쓰기 때문이다.
     나키()가 비면(서버 꺼짐) 옛 규칙 그대로다 — 이 기기 안에서만 도는 판이라 부딪힐 상대가 없다. */
  function 메일조각(m) {
    return String(m || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
  }
  function 일지id(날짜) {
    var 조각 = 메일조각(나());
    return 조각 ? ('일지-' + 날짜 + '-' + 조각) : ('일지-' + 날짜);
  }

  function 새id(종류, 날짜) {
    return 종류 === '일지' ? 일지id(날짜) : 'm-' + Date.now().toString(36) + '-' + 난수(4);
  }

  /* 그 날 내 일지 한 장. 🔴 규칙이 바뀌기 전에 쓴 옛 줄(`일지-<날짜>`)도 찾아 준다 —
     안 그러면 달력엔 점이 찍히는데 눌러 보면 빈 일지가 뜬다. 내 것이거나 임자 없는 것만 집는다 */
  function 일지한장(날짜) {
    var 새것 = 한장(일지id(날짜));
    if (새것) return 새것;
    var 옛것 = 한장('일지-' + 날짜);
    if (옛것 && 내것인가(옛것)) return 옛것;
    return null;
  }
  ...
  /* ── 누구 것인가 ── (2026-09-17 우람님)
     메모와 체크리스트는 **각자 제 것만** 본다. 규칙은 하나다 —
     🔴 쓴이가 없는 줄은 모두에게, 쓴이가 있는 줄은 그 사람에게만.
     그래서 옛 줄은 손대지 않아도 그대로 모두에게 남는다 (옮겨 심는 스크립트가 없다).
     🔴 영농일지도 나눈다 (2026-09-17 우람님이 넷 다 개인화하라 하셨다).
        나누려면 id 에 사람이 들어가야 한다 — 일지id() 참조.
  ...
  function 저장(값) {
    ...
    /* 🔴 새로 만드는 줄에만 쓴이를 찍는다. 옛 줄을 고쳐 저장해도 안 찍는다 —
       찍는 순간 남들 화면에서 그 메모가 사라진다.
       메모·체크·일지 넷 다 같은 규칙이다 (2026-09-17 우람님: 메모탭 전부 개인화) */
    if (!레코드.쓴이) {
      var 나메일 = 나();
      if (나메일) 레코드.쓴이 = 나메일;
    }
  ...
  ZG.메모자료 = {
    ...
    한장: 한장, 일지한장: 일지한장, 일지id: 일지id, 목록: 목록, 날짜묶기: 날짜묶기,
    폴더세기: 폴더세기, 일지날들: 일지날들,
    상태들: 상태들, 상태정규화: 상태정규화,
