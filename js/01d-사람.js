...
(function (ZG) {
  ...
  /* ══════════════════════════════════════════════════════════════
     씨앗 — 미리 정해 두는 사람들. 두 갈래다.

     ① 이름씨앗 — **가입할 때 넣은 이름**으로 닉네임과 사진을 붙인다 (2026-09-17 우람님).
        이메일은 미리 알 수 없어도 이름은 안다. 그 사람이 가입해 처음 들어오는 순간
        닉네임(화면에 뜨는 이름)과 프로필 사진이 이미 들어가 있다.
        🔴 사진은 저장소에 커밋한 파일이다 — img/사람/<본명>.png.
           우람님 바탕화면 「정원사의 널서리」 폴더의 것을 256px 로 줄여 넣었다.
        본인이 프로필에서 바꾸면 그때부터 그 값이 이긴다. 씨앗이 다시 덮지 않는다.

     ② 메일씨앗 — 이메일을 아는 사람을 통째로 미리 넣어 둘 때 쓴다. 지금은 비어 있다.
     ══════════════════════════════════════════════════════════════ */
  var 이름씨앗 = [
    { 이름: '강성군', 닉: '강PD',   사진: 'img/사람/강성군.png' },
    { 이름: '서수현', 닉: '서작가', 사진: 'img/사람/서수현.png' },
    { 이름: '유한누리', 닉: '바오밥', 사진: 'img/사람/유한누리.png' },
    { 이름: '조수형', 닉: '조사장', 사진: 'img/사람/조수형.png' },
    { 이름: '백우람', 닉: '우람씨', 사진: 'img/사람/백우람.png' },
    { 이름: '김두용', 닉: '두용씨', 사진: 'img/사람/김두용.png' }
  ];

  var 씨앗 = [
    // { 메일: 'kim@example.com', 이름: '김철수', 직책: '대표', 사진: 'img/사람/김철수.png' }
  ];

  var 씨앗표 = {};
  씨앗.forEach(function (s) { 씨앗표[String(s.메일).toLowerCase()] = s; });

  /* 가입할 때 넣은 이름으로 찾는다. 사이 공백은 무시한다 —「유 한누리」로 넣어도 걸리게 */
  function 이름눌러(v) { return String(v || '').replace(/\s+/g, ''); }
  var 이름표 = {};
  이름씨앗.forEach(function (s) { 이름표[이름눌러(s.이름)] = s; });
  function 이름씨(본명) { return 이름표[이름눌러(본명)] || null; }
  ...
  function 세션(사용자) {
    ...
    var 본명 = m.이름 || '';
    var 씨 = 씨앗표[메일] || {};
    var 이씨 = 이름씨(본명);        // 가입 때 넣은 이름으로 찾은 닉네임·사진

    var 있던것 = 한장(메일);
    if (있던것) {
      /* 🔴 이미 들어와 있던 사람도 한 번은 챙긴다 — 닉네임 규칙이 생기기 전에 가입한 분들이다.
         단 **아직 사진이 없고, 화면 이름이 가입 때 넣은 본명 그대로일 때만** 올린다.
         본인이 이미 바꿔 둔 것을 덮지 않기 위해서다. 한 번 올리면 이름이 닉으로 바뀌어
         다음부터는 이 조건에 안 걸린다. */
      if (이씨 && !있던것.사진 && 이름눌러(있던것.이름) === 이름눌러(본명)) {
        저장(메일, { 이름: 이씨.닉, 직책: 있던것.직책 || m.직책 || '', 사진: 이씨.사진 });
        다시그리기();
      }
      return;
    }

    // 내 줄이 아직 없다 — 이름씨앗 → 메일씨앗 → 가입 때 넣은 값 차례로 본다
    저장(메일, {
      이름: (이씨 && 이씨.닉) || 씨.이름 || 본명 || 메일.split('@')[0],
      직책: 씨.직책 || m.직책 || '',
      사진: (이씨 && 이씨.사진) || 씨.사진 || ''
    });
    /* 방금 나를 만들었다 — 이미 그려진 화면에는 내 얼굴도 이름도 없다.
       곁줄의 내 자리, 업무 지정 고르개가 이때 채워진다 */
    다시그리기();
  }
  ...
  function 내시트() {
    var 알 = ZG.알림;
    var 항목 = [{ 값: '프로필', 그림: '사람', 글: '내 프로필' }];
    // 알림 — 이 기기에서 쓸 수 있을 때만 낸다. 못 쓰는 기기에 안 되는 단추를 두지 않는다
    if (알 && 알.될까()) {
      항목.push({ 값: '알림', 그림: '종', 글: 알.켜져있나() ? '알림 끄기' : '알림 켜기' });
    }
    항목.push({ 값: 'PC보기', 그림: u.PC보기인가() ? '폰' : '화면', 글: u.PC보기인가() ? '폰화면으로' : 'PC화면으로' });
    항목.push({ 값: '로그아웃', 그림: '나감', 글: '로그아웃' });

    u.고르기({ 제목: 이름(나키()), 항목: 항목 }, function (값) {
      if (값 === '프로필') 프로필창();
      else if (값 === '알림') { 알.켜져있나() ? 알.끄기() : 알.켜기(); }
  ...
  /* 프로필을 바꾸면 화면마다 붙은 얼굴을 새로 그려야 한다.
     🔴 홈은 온판으로 다시 그린다 — 얼굴이 머리줄과 곁줄에도 붙어 있어 피드만 갈아서는 안 바뀐다.
        부르는 자리가 둘뿐이다(첫 로그인 직후 · 프로필 저장 직후) — 둘 다 글을 치던 중이 아니다. */
  function 다시그리기() {
    ['앱', '업체앱', '메모앱', '홈앱'].forEach(function (이름) {
      var a = ZG[이름];
      if (!a) return;
      var 그리기 = a.온판다시 || a.다시그리기;
      if (typeof 그리기 === 'function') { try { 그리기(); } catch (e) { console.warn(e); } }
    });
  }
  ...
  /* 🔴 내가 누구인지 **스스로** 챙긴다 (2026-09-17 고침).
     01b 도 세션을 받고 나서 불러 주지만, 그것만 믿으면 안 된다 —
     supabase-js 의 getSession() 은 localStorage 에서 바로 답을 준다. 네트워크를 안 기다리므로
     그 .then 이 **다음 <script> 태그가 실행되기도 전에** 돌아버린다.
     그때 이 파일(01d)은 아직 안 실려 있어 `ZG.사람` 이 없고, 01b 의 가드가 조용히 건너뛴다.
     그 바람에 gn_사람 이 텅 비고 채팅·업무의 쓴이가 전부 빈 채로 쌓였다.
     여기서 한 번 더 물어보면 싣는 차례가 어떻든 상관이 없다. 두 번 불려도 한장()이 막는다. */
  (function 스스로챙기기() {
    var 서 = ZG.서버;
    if (!서 || !서.클라이언트 || !서.클라이언트.auth) return;
    서.클라이언트.auth.getSession().then(function (r) {
      if (r && r.data && r.data.session) 세션(r.data.session.user);
    }).catch(function (e) { console.warn('내가 누구인지 못 챙겼습니다', e); });
  })();
})(window.ZG);
